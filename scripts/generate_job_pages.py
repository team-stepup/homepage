...
def parse_wage(wage):
    """'1,300円〜1,625円'→(1300,1625) / '1,350円〜'→(1350,None)
    / '1,200円'→(1200,None) / 数字なし→None"""
    wage = wage or ""
...
def valid_through(updated):
    """掲載有効期限 = 更新日+180日。求人が更新されれば自動延長される
    (入力が同じなら出力も同じ、の決定性を保つため現在時刻は使わない)"""
    try:
        d = datetime.date.fromisoformat((updated or "").strip())
    except ValueError:
        return None
    return (d + datetime.timedelta(days=180)).isoformat()
...
def build_ld(j):
    ...
    vt = valid_through(j.get("updated"))
    if vt:
        ld["validThrough"] = vt
    if j.get("hours"):
    ...
    if w:
        mn, mx = w
        # Google仕様: value 単独 か minValue+maxValue のペアのみ
        # (minValue 単独は「maxValueがありません」警告になる)
